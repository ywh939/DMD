...
def compute_statistics_jit(overlaps,
    ...
    if compute_fp:
        for i in numba.prange(det_size):
    ...
    for i in numba.prange(gt_size):
        ...
        for j in numba.prange(det_size):
            if (ignored_det[j] == -1):
                continue
    ...
    if compute_fp:
        ...
        if metric == 0:
            ...
            for i in numba.prange(dc_bboxes.shape[0]):
                for j in numba.prange(det_size):
                    if (assigned_detection[j]):
                        continue
...
def fused_compute_statistics(overlaps,
    ...
    dt_num = 0
    dc_num = 0
    for i in numba.prange(gt_nums.shape[0]):
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    mAP3d, mAP3d_R40 = do_eval(gt_annos, dt_annos, current_classes, min_overlaps, PR_detail_dict=PR_detail_dict)
